tasks {
    ...
    register("buildApp") {
        //dependsOn(gradle.includedBuild("android").task(":assemble"  ))
        dependsOn(gradle.includedBuild("desktop").task(":runtimeZip"))
        //dependsOn(gradle.includedBuild("ios"    ).task(":build"     ))
        dependsOn(gradle.includedBuild("server" ).task(":war"       ))
    }
